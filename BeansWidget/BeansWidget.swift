...
private struct BeansWidgetProvider: TimelineProvider {
    ...
    private var sampleEntry: BeansWidgetEntry {
        BeansWidgetEntry(date: Date(), title: "Midnight City", artist: "M83", album: "Hurry Up, We're Dreaming", isPlaying: true)
    }
}
...
private extension BeansWidgetView {
    var cover: some View {
        ZStack {
            LinearGradient(colors: [Color(red: 0.96, green: 0.18, blue: 0.16), Color(red: 0.28, green: 0.03, blue: 0.04)], startPoint: .topLeading, endPoint: .bottomTrailing)
    ...
    private var songText: some View {
        VStack(alignment: .leading, spacing: 3) {
            Text(entry.title).font(.system(size: 15, weight: .semibold)).foregroundStyle(.white).lineLimit(1)
            Text(entry.artist).font(.system(size: 13)).foregroundStyle(.white.opacity(0.62)).lineLimit(1)
        }
    }
    ...
    private var smallLayout: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                songText
                HStack {
                    Text("BEANS MUSIC").font(.system(size: 9, weight: .bold)).tracking(0.5).foregroundStyle(.white.opacity(0.62))
                    Spacer()
                    playButton
                }
            }
    ...
    private var mediumLayout: some View {
        HStack(spacing: 14) {
            cover.frame(width: 84, height: 84)
            VStack(alignment: .leading, spacing: 10) {
                songText
                Text(entry.album).font(.system(size: 11)).foregroundStyle(.white.opacity(0.45)).lineLimit(1)
                HStack(spacing: 18) {
    ...
    private var largeLayout: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Label("BEANS MUSIC", systemImage: "waveform").font(.system(size: 11, weight: .bold)).foregroundStyle(.white.opacity(0.58))
                Spacer()
                Image(systemName: "ellipsis").foregroundStyle(.white.opacity(0.62))
            }
            HStack(spacing: 14) {
                cover.frame(width: 112, height: 112)
                VStack(alignment: .leading, spacing: 6) {
                    songText
                    Text(entry.album).font(.system(size: 12)).foregroundStyle(.white.opacity(0.48)).lineLimit(2)
                }
                Spacer(minLength: 0)
            }
            VStack(spacing: 5) {
                Capsule().fill(.white.opacity(0.22)).frame(height: 3).overlay(alignment: .leading) {
                    Capsule().fill(.white).frame(width: 78, height: 3)
                }
                HStack {
                    Text("1:24")
                    Spacer()
