...
- 播放器封面页新增歌名颜色和预览歌词颜色自定义，留空时自动跟随封面取色。
- 内置两个来自 LX Music 音源仓库的预设：统一音乐源 · GD Studio、忆音音源 · DreamMeting。

### 修复

- 修复播放中进度更新过于频繁导致非播放器页面也跟随重绘的问题。
- 修复重新上传歌词背景或恢复壁纸后，部分位置可能继续显示旧图片缓存的问题。
- 修复酷狗音乐排行榜详情歌曲没有封面，导致播放器和锁屏无法显示封面的问题。
- 修复酷狗排行榜只返回首页 30 首歌曲的问题，并加入分页加载与重复结果保护。
- 优化首次启动时字体注册和壁纸恢复时机，降低巨魔用户安装或首次启动卡死风险。
