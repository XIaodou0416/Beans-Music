...
enum ChangelogStore {
    ...
    static let logs: [VersionLog] = [
        VersionLog(
            ...
            title: "主页、播放器与本地歌单优化",
            features: [
                "主页问候文字支持多行输入，第一至第三行可分别调节字号、颜色和上下偏移",
                "打开设置界面时暂停主页内容渲染和数据刷新，减少设置页卡顿与额外发热",
                "左右滑动切换歌曲，左滑下一首、右滑上一首",
                "本地歌单多选后支持批量添加到其他本地歌单",
                "一键同步歌单支持自选两个或三个平台，并合并创建对应的本地歌单",
                "设置页改为全屏显示，打开后不再保留顶部空白",
                "主页问候文字支持开启背景发光，并可调节发光强度",
                "主页问候文字支持专属字体、底部横线和上下渐变，并可分别选择渐变起始与结束颜色"
            ],
            fixes: [
                "修复多行问候文字无法逐行调整的问题",
                "修复本地歌单多选操作只能移除歌曲的问题",
                "修复同步平台只能单选、无法合并两个平台喜欢歌曲的问题",
                "扩充内置音源密钥池，增加两个可用密钥"
            ]
        ),
        VersionLog(
