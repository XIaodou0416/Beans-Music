...
enum ChangelogStore {
    ...
    static let logs: [VersionLog] = [
        VersionLog(
            ...
            title: "高刷新与发布整理",
            features: [
                "强制开启全局 120Hz 高刷新请求，修复“我的”、设置、播放器等页面被稳定限制在 60Hz 的问题",
                "低系统悬浮底栏调节项仅在 iOS 26 以下显示，iOS 26 及以上继续使用系统原生底栏",
                "自定义布局里的组件大小支持缩到更小，方便把顶部、底部和封面页控件调成极简布局",
                "播放页顶部、底部与封面页组件支持更完整的颜色调节",
                "内置音源调用池扩容，增加更多可用调用额度"
            ],
            fixes: [
                "延续 QQ 音乐微信登录后的歌单与收藏同步修复，补充更多账号身份候选"
            ]
        ),
        VersionLog(
            id: "1.5.6",
