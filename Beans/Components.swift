...
struct BeansNowPlayingPresentation<Content: View>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack(alignment: .top) {
                ...
                if isPhone {
                    dragIndicator(
                        ...
                        targetOffset: proxy.size.height * 0.82
                    )
                }
            }
            ...
            .scaleEffect(
                usesSystemInteractiveDismissal ? 1 : presentationScale,
                anchor: .top
            )
            .clipShape(
                RoundedRectangle(
                    cornerRadius: usesSystemInteractiveDismissal ? 0 : presentationCornerRadius,
                    style: .continuous
                )
            )
            // 详情页的 fullScreenCover 不一定提供系统下划手势。
            // 把手势挂在整个顶层容器上，但只接受从顶部开始的竖直下划，
            // 这样歌单列表/歌词滚动仍由各自的 ScrollView 处理。
            .simultaneousGesture(
                dismissGesture(
                    topEdge: proxy.safeAreaInsets.top + 132,
                    targetOffset: proxy.size.height * 0.82
                )
            )
        }
    ...
    private var presentationCornerRadius: CGFloat {
        presentationProgress * 28
    }

    @ViewBuilder
    private func dragIndicator(safeAreaTop: CGFloat, targetOffset: CGFloat) -> some View {
        let surface = ZStack(alignment: .top) {
            // Keep the hit area at the physical top edge. The visible indicator
            // may have a custom layout offset, but that must not move the swipe
            // target off-screen.
            Color.clear
                .frame(
        ...
        .highPriorityGesture(
            dismissGesture(
                topEdge: safeAreaTop + BeansNowPlayingPresentationMetrics.indicatorHitHeight,
    ...
    private func dismissGesture(topEdge: CGFloat, targetOffset: CGFloat) -> some Gesture {
        DragGesture(minimumDistance: 3, coordinateSpace: .global)
            .onChanged { value in
                guard !usesSystemInteractiveDismissal,
                      value.startLocation.y <= topEdge,
            ...
            .onEnded { value in
                ...
                if translation > BeansNowPlayingPresentationMetrics.dismissDistance
                    ...
                    withAnimation(BeansNowPlayingPresentationMetrics.dismissAnimation) {
                        dragOffset = max(targetOffset, translation + 120)
                    }
