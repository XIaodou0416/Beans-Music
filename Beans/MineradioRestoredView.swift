    ...
    final class Coordinator: NSObject, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler {
        ...
        private func perform(action: String, payload: [String: Any]) async throws -> [String: Any] {
            ...
            case "netease-song-url":
                ...
                let quality = value(payload, ["quality", "level"])
                let info = try await NetEaseAPI.shared.songURLInfo(ids: [id], level: quality.isEmpty ? BeansAudioQuality.current.level : quality)[id]
                let url = info?.url ?? ""
                return [
                    "provider": "netease",
                    "url": url,
                    ...
                    "level": quality.isEmpty ? BeansAudioQuality.current.level : quality,
                ]
            ...
            case "qq-song-url":
                ...
                let url = try await QQMusicAPI.shared.songURL(
                    ...
                    quality: audioQuality(from: value(payload, ["quality", "level"]))
                ) ?? ""
